...
def _allow_insecure_tls_fallback() -> bool:
    raw = str(os.environ.get("RSM_INSECURE_TLS_FALLBACK", "1")).strip().lower()
...
def _urlopen_with_tls_fallback(req: urllib.request.Request, *, timeout: float):
    try:
        return urllib.request.urlopen(req, timeout=timeout)  # noqa: S310
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, OSError) as e:
        if not (_allow_insecure_tls_fallback() and _is_cert_verify_error(e)):
            raise
        log.warning(
            "TLS verify failed for %s; retrying with insecure TLS fallback (RSM_INSECURE_TLS_FALLBACK=1).",
            req.full_url,
        )
        insecure_ctx = ssl._create_unverified_context()
        return urllib.request.urlopen(req, timeout=timeout, context=insecure_ctx)  # noqa: S310
...
def download_reshade_installer(version: str, paths: RsmPaths, *, addon: bool) -> Path:
    ...
    try:
        with _urlopen_with_tls_fallback(req, timeout=120) as resp:
...
def _ensure_minimal_reshade_ini_if_missing(manifest: GameManifest, game_dir: Path) -> None:
    ...
    ini_path.write_text(_MINIMAL_RESHADE_INI_DECKY, encoding="utf-8")
